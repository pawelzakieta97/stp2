...
PID2legend = ['regulator PID dobrany rêcznie Kr=',num2str(Kr2),' Td=',num2str(Td2),' Ti=',num2str(Ti2)];
DMClegend = ['regulator DMC D=',num2str(D),' N=',num2str(N),' Nu=',num2str(Nu),' lambda=',num2str(lambda)];
legend('wartoœæ zadana',PIDZNlegend,PID2legend,DMClegend);
title(['Sygna³ wyjœciowy odpowiedzi skokowej modelu',newline,'w zale¿noœci od regulatora']);
xlabel('t');
ylabel('y(t)');
fig = gcf;
...
DMClegend = ['regulator DMC D=',num2str(D),' N=',num2str(N),' Nu=',num2str(Nu),' lambda=',num2str(lambda)];
legend('wartoœæ zadana',PIDZNlegend,PID2legend,DMClegend);
title(['Sygna³ steruj¹cy odpowiedzi skokowej modelu',newline,'w zale¿noœci od regulatora']);
xlabel('t');
ylabel('y(t)');
fig = gcf;
fig.Position = [10,10,900,600];
